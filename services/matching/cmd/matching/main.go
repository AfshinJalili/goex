...
import (
	...
	"github.com/AfshinJalili/goex/services/matching/internal/config"
	"github.com/AfshinJalili/goex/services/matching/internal/consumer"
	"github.com/AfshinJalili/goex/services/matching/internal/engine"
	"github.com/AfshinJalili/goex/services/matching/internal/service"
	"github.com/AfshinJalili/goex/services/matching/internal/storage"
...
func main() {
	...
	matchingMetrics := service.NewMetrics(registry)
	...
	snapshotStore := storage.NewSnapshotStore(pool)
	...
	go func() {
		logger.Info("matching grpc starting", "addr", grpcAddr)
	...
	go func() {
		logger.Info("matching http starting", "addr", httpServer.Addr)
	...
	go func() {
		logger.Info("matching consumer starting", "topics", []string{cfg.Kafka.Topics.OrdersAccepted, cfg.Kafka.Topics.OrdersCancelled})
		if err := consumerGroup.Consume(consumerCtx, []string{cfg.Kafka.Topics.OrdersAccepted, cfg.Kafka.Topics.OrdersCancelled}, orderConsumer); err != nil {
