...
import (
	...
	"github.com/AfshinJalili/goex/services/fee/internal/cache"
	"github.com/AfshinJalili/goex/services/fee/internal/config"
	"github.com/AfshinJalili/goex/services/fee/internal/service"
...
func main() {
	...
	feeMetrics := service.NewMetrics(registry)
	...
	cacheStore := cache.NewTierCache()

	if err := loadCache(cacheStore, store, feeMetrics, logger); err != nil {
	...
	feepb.RegisterFeeServiceServer(grpcServer, feeService)
	...
	go func() {
		logger.Info("fee grpc starting", "addr", grpcAddr)
	...
	go func() {
		logger.Info("fee http starting", "addr", httpServer.Addr)
...
func loadCache(cacheStore *cache.TierCache, store *storage.Store, feeMetrics *service.Metrics, logger *slog.Logger) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	start := time.Now()
	if err := cacheStore.Load(ctx, store); err != nil {
		return err
	}
	if feeMetrics != nil {
